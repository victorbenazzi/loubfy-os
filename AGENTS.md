...
- `.claude/` for Claude Code. `.claude/skills/` links to `skills/` (generated by `npm install`, works on macOS/Linux/Windows), `agents/roster/` roles are exposed as subagents in `.claude/agents/`, `/onboarding` is registered in `.claude/commands/`, and `.claude/settings.json` adds a hook that protects `references/templates/`.
